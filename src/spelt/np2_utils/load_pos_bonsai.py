...
def load_pos_bonsai_roi(path, ppm, trial_type):
    """
    Load position data from a .csv file generated by Bonsai (ROI-based tracking)
    Assumes columns are structured as follows:
    -
    ...
    elif trial_type == "plus-maze":
        frame_count = data.loc[:, "Value.Item1"]
        pointgrey_timestamps = data.loc[:, "Value.Item2"]
        position = data.loc[:, ["Value.Item3.X", "Value.Item3.Y"]].T
        bonsai_timestamps = data.loc[:, "Timestamp"]
        ppm = 1000  # Estimate based on typical setup
        print("Estimating PPM for plus-maze at 1000 based on typical setup")
        x_min = 0
        x_max = 1000
        y_min = 0
        y_max = 1000
        maze_roi = data.loc[:, "Value.Item4"] if "Value.Item4" in data.columns else None
        maze_state = None

    elif trial_type == "sleep-box":
        pointgrey_timestamps = data.loc[:, "Value.Item1"]
        frame_count = data.loc[:, "Value.Item2"]
        position = data.loc[:, ["Value.Item3.X", "Value.Item3.Y"]].T
        bonsai_timestamps = data.loc[:, "Timestamp"]
        ppm = 840
        print("Estimating PPM for sleep-box at 840 based on typical setup")
        x_min = 0
        x_max = 520
        y_min = 0
        y_max = 520
...
def load_pos_bonsai_leds(path, ppm, trial_type):
    """
    Load position data from Bonsai LED tracking CSV format.
    ...
    if "Value.Item1.Item1" in data.columns:
        # Nested format: Value.Item1.Item1, Value.Item1.Item2, etc.
        col_map = {
            "timestamp": "Value.Item1.Item1",
            "frame_count": "Value.Item1.Item2",
            "led1_x": "Value.Item1.Item3.X",
            "led1_y": "Value.Item1.Item3.Y",
            "led2_x": "Value.Item1.Item4.X",
            "led2_y": "Value.Item1.Item4.Y",
            "ppm": "Value.Item1.Item5.Item1",
        }
    elif "Value.Item1" in data.columns:
        # Flat format: Value.Item1, Value.Item2, Value.Item3.X, etc.
        col_map = {
            "timestamp": "Value.Item1",
            "frame_count": "Value.Item2",
            "led1_x": "Value.Item3.X",
            "led1_y": "Value.Item3.Y",
            "led2_x": "Value.Item4.X",
            "led2_y": "Value.Item4.Y",
            "ppm": "Value.Item5.Item1",
        }
    else:
        raise ValueError(
            f"Unrecognized CSV format. Expected columns starting with "
            f"'Value.Item1.Item1' (nested) or 'Value.Item1' (flat). "
            f"Found columns: {list(data.columns[:5])}"
        )

    # Add if statement to check if the ppm is in the sheet or not
    if col_map["ppm"] in data.columns:
        # Extract the first value from the column (since it's consistent)
        ppm_value_from_csv = data[col_map["ppm"]].iloc[0]
        ppm_values = {
            "baseline": ppm_value_from_csv,
            "similar": ppm_value_from_csv,
            "different": ppm_value_from_csv,
            "sleep": ppm_value_from_csv,
            "plus-maze": ppm_value_from_csv,
            "open-field": ppm_value_from_csv,
            "sleep-box": ppm_value_from_csv,
        }
    else:
    ...
    pointgrey_timestamps = data.loc[:, col_map["timestamp"]]
    frame_count = data.loc[:, col_map["frame_count"]]
    bonsai_timestamps = data.loc[:, "Timestamp"]
