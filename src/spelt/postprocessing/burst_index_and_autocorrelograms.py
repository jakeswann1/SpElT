...
def compute_autocorrelogram(cluster_spike_times, bin_edges, time_window):
    """
    Compute autocorrelogram for a single cluster.

    Parameters:
    - cluster_spike_times (numpy array): Sorted spike times for a single cluster.
    - bin_edges (numpy array): Edges of bins for the histogram.
    - time_window (float): Time window in seconds.

    Returns:
    - bin_centers (numpy array): Centers of the bins.
    - counts (numpy array): Counts in each bin.
    """
...
def compute_first_moment(bin_centers, counts):
    """
    Compute the first moment (mean) of positive bins in an autocorrelogram.

    Parameters:
    - bin_centers (numpy array): Centers of bins in seconds.
    - counts (numpy array): Counts in each bin.

    Returns:
    - first_moment (float): First moment in milliseconds, or np.nan if
        no positive counts.
    """
    # Convert bin centers to milliseconds
    bin_centers_ms = bin_centers * 1000

    # Filter bins where bin centers are positive
    positive_indices = np.where(bin_centers_ms > 0)[0]

    if len(positive_indices) == 0:
        return np.nan

    positive_bin_centers = bin_centers_ms[positive_indices]
...
def compute_autocorrelograms_and_first_moment(
    spike_times, spike_clusters, bin_size, time_window
):  # , burst_threshold):
    """
    Compute autocorrelograms and first moments for multiple clusters.

    Adapted function to compute autocorrelograms, burst indices, and first
    moments (in ms) for multiple clusters, returning them as separate
    dictionaries. Includes acWin to constrain bins for first moment
    calculation.

    Parameters:
    - spike_times (numpy array): Array of spike times in seconds.
    - spike_clusters (numpy array): Array of cluster IDs corresponding
        to each spike time.
    - bin_size (float): The size of the bins for the autocorrelogram
        in seconds.
    - time_window (float): The time window around each spike to consider
        for autocorrelation and first moment in seconds.

    Returns:
    - autocorrelograms (dict): A dictionary containing the bin centers
        and counts for each cluster's autocorrelogram.
    - first_moments (dict): A dictionary containing the first moment
        for each cluster's autocorrelogram (in ms).
    """
    ...
    for cluster in unique_clusters:
        ...
        bin_centers, counts = compute_autocorrelogram(
            cluster_spike_times, bin_edges, time_window
        )

        # Store autocorrelogram in dictionary
        autocorrelograms[cluster] = {"bin_centers": bin_centers, "counts": counts}

        # Compute first moment (mean) of the autocorrelogram in milliseconds
        first_moment = compute_first_moment(bin_centers, counts)

        # Store first moment in dictionary
        first_moments[cluster] = first_moment
...
def plot_autocorrelogram(
    session, cluster, autocorrelogram, first_moment
):  # burst_index,
    """
    Plot an autocorrelogram for a given cluster with the burst index.

    Parameters:
    - cluster (int): Cluster ID.
    - autocorrelogram (dict): A dictionary containing the bin centers
        and counts for the cluster's autocorrelogram.
    - burst_index (float): The burst index for the cluster.
    - first_moment (float): The first moment of the autocorrelogram
        for the cluster.

    Returns:
    - fig, ax (matplotlib figure and axis): Figure and axis containing
        the plot.
    """
    fig, ax = plt.subplots()
    ...
    ax.bar(
        autocorrelogram["bin_centers"], autocorrelogram["counts"], width=0.001
    )  # Assuming bin size of 1ms
    ax.set_title(
        f"{session} Cluster {cluster}:  First Moment = {first_moment:.3f}"
    )  # Burst Index = {burst_index:.3f};
    ax.set_xticklabels(
        np.round(ax.get_xticks() * 1000)
    )  # Convert x tick values to milliseconds
...
def plot_autocorrelograms_with_dropdown(autocorrelograms):
    """
    Plot autocorrelograms for each cluster using a dropdown widget.

    Note: This function is intended to be run in a Jupyter Notebook
    environment with ipywidgets installed.
    ...
    from ipywidgets import (
        interact,
        widgets,
    )  # Importing widgets within the function for portability
