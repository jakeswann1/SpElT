...
def get_theta_phase(
    lfp, sampling_rate, peak_freq, clip_value=0, filt_half_bandwidth=2, power_thresh=5
):
    """
    ...
    pass_band = np.array(
        [peak_freq - filt_half_bandwidth, peak_freq + filt_half_bandwidth]
    )
    cycle_length_lim = np.ceil(1 / pass_band * sampling_rate).astype(int)
    bad_length_cycles = (cycle_lengths < cycle_length_lim[1]) | (
        cycle_lengths > cycle_length_lim[0]
    )
    ...
    cycle_numbers = cycle_numbers.astype(
        float
    )  # Convert to float to allow incluion of NaN
    cycle_numbers[bad_cycle_indices] = np.nan

    return theta_phase, cycle_numbers


def get_spike_theta_phase(
    lfp,
    spike_times,
    sampling_rate,
    peak_freq,
