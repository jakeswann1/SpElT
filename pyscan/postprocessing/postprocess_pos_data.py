...
def write_csv_from_pos(file_path):
    ...
    pos_df = pd.DataFrame([framecounter, x1s, x2s, y1s, y2s, numpix1s, numpix2s, totalpixs], 
                          index = ['Framecounter', 'X1', 'X2', 'Y1', 'Y2', 'Pixels LED 1', 'Pixels LED 2', 'Total Pixels'])
...
def led_swap_filter(led_pos, led_pix, thresh=5):
    ...
    dist12 = np.sqrt((led_pos[0, pos] - led_pos[2, pos-1])**2 + (led_pos[1, pos] - led_pos[3, pos-1])**2)
    dist11 = np.sqrt((led_pos[0, pos] - led_pos[0, pos-1])**2 + (led_pos[1, pos] - led_pos[1, pos-1])**2)
    dist21 = np.sqrt((led_pos[2, pos] - led_pos[0, pos-1])**2 + (led_pos[3, pos] - led_pos[1, pos-1])**2)
    dist22 = np.sqrt((led_pos[2, pos] - led_pos[2, pos-1])**2 + (led_pos[3, pos] - led_pos[3, pos-1])**2)

    # Determine if a swap has occurred
    switched = (dist12 < dist11-thresh) & (np.isnan(led_pos[2, pos]) | (dist21 < dist22-thresh))
...
def process_position_data(posdata, max_speed, smoothing_window_size):
    ...
    direction = np.mod((180/np.pi) * (np.arctan2(-led_pos.iloc[1, :] + led_pos.iloc[3, :], led_pos.iloc[0, :] - led_pos.iloc[2, :])) - correction, 360)
