...
def spatial_info(rate_maps, pos_map):
    ...
    if single_pos_map:
        ...
        if isinstance(pos_map, np.ndarray) and pos_map.ndim == 3:
            ...
            raise ValueError("pos_map should be a dict, list, 3D ndarray, or 2D ndarray.")
    ...
    mean_rates = np.nansum(rate_maps_array * pos_maps_array, axis=(1, 2)) / total_occupancy
    ...
    with np.errstate(divide='ignore', invalid='ignore'):
        bits_per_sec_array = np.array([np.nansum(p_x[i] * rate_maps_array[i] * np.log2(p_r[i]))
                                       if mean_rates[i] != 0 else 0
