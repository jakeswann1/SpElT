...
def make_direction_rate_map(spike_times, frame_times, directions, n_bins):
    ...
    if spike_times.ndim != 1 or frame_times.ndim != 1 or directions.ndim != 1:
        raise ValueError("spike_times, frame_times, and directions must be 1-dimensional")
    ...
    bin_labels = [f'{direction_bins[i]:.1f}-{direction_bins[i+1]:.1f}' for i in range(len(direction_bins)-1)]
